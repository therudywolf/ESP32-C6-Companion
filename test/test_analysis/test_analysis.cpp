...
static void test_trough_passed_beats_still_falling() {
  ...
  int n = analysis::analyse(w, 220, 45, 12, -1, gF, 8);
...
static void test_accelerating_and_easing_are_opposites() {
  ...
  n = analysis::analyse(w, 220, 45, 12, -1, gF, 8);
...
static void test_storm_needs_the_wmo_very_rapid_band() {
  ...
  TEST_ASSERT_FALSE(
      has(analysis::analyse(w, 220, 45, 12, -1, gF, 8),
...
static void test_unset_windows_produce_nothing() {
  ...
  analysis::Windows w;
  int n = analysis::analyse(w, -32768, -1, 12, -1, gF, 8);
...
static void test_ventilation_needs_pressure_to_vouch_for_it() {
  ...
  TEST_ASSERT_TRUE(has(analysis::analyse(w, 200, 45, 12, -1, gF, 8),
                       analysis::PAT_VENTILATION));

  w.dP10_1h = -20; /* the atmosphere IS moving: not a window */
  TEST_ASSERT_FALSE(has(analysis::analyse(w, 200, 45, 12, -1, gF, 8),
                        analysis::PAT_VENTILATION));
}
...
static void test_condensation_fires_on_the_dew_point_not_the_humidity() {
  ...
  analysis::Windows w;
  TEST_ASSERT_FALSE(has(analysis::analyse(w, 240, 60, 12, -1, gF, 8),
                        analysis::PAT_CONDENSATION));
  int n = analysis::analyse(w, 120, 85, 12, -1, gF, 8);
...
static void test_cold_night_is_only_said_while_it_is_still_actionable() {
  ...
  TEST_ASSERT_FALSE(has(analysis::analyse(w, 220, 45, 9, -1, gF, 8),
                        analysis::PAT_RADIATIVE_NIGHT));
  TEST_ASSERT_TRUE(has(analysis::analyse(w, 220, 45, 19, -1, gF, 8),
                       analysis::PAT_RADIATIVE_NIGHT));
}

static void test_percentile_extremes_only() {
  analysis::Windows w;
  TEST_ASSERT_TRUE(has(analysis::analyse(w, 220, 45, 12, 2, gF, 8),
                       analysis::PAT_PRESSURE_LOW));
  TEST_ASSERT_TRUE(has(analysis::analyse(w, 220, 45, 12, 98, gF, 8),
                       analysis::PAT_PRESSURE_HIGH));
  /* Mid-range says nothing, and -1 (archive too short) says nothing either. */
  int n = analysis::analyse(w, 220, 45, 12, 50, gF, 8);
...
static void test_output_is_capped_not_overflowed() {
  ...
  int n = analysis::analyse(w, 120, 90, 20, 1, small, 2);
